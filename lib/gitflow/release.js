...
config.isMergeableBranch = config.isDevelop || config.isHotfix || config.isBugfix || config.isFeature || config.isRelease;
...
================================================================`);
        ...
        if (config.isDevelop) {
            ...
            logger.info(`Pushing branch ${config.releaseBranchName} to origin`);
        ...
        if (config.isRelease) {
            logger.info(`Merging ${config.targetBranch} into master`);
            const releaseTagName = config.targetBranch.split('release/')[1];
            execOrFail('git checkout develop && git fetch && git pull');
            execOrFail(`git merge --no-ff --no-edit -m "Merge ${config.targetBranch} into master" ${config.targetBranch}`);

            logger.info('Creating and pushing tag');
            execOrFail(`git tag -a ${releaseTagName} -m "Merge ${config.targetBranch} into master"`);
            execOrFail(`git push origin ${releaseTagName}`);
